...
namespace MMC_Software
{
    ...
    public partial class VentanaRegistroCompras : UserControl
    {
        ...
        int? _AlmacenID = 0;
        public int _CompraID = 0;
        #endregion

        public VentanaRegistroCompras(int? VendedorID, int? AlmacenID, bool Modificando , int CompraID=0)
        {
            InitializeComponent();
            _AlmacenID = AlmacenID;
            _VendedorID = VendedorID;
            _Modificando = Modificando;
            _CompraID = CompraID;
            ...
            Loaded += VentanaRegistroCompras_Loaded;
        }
        ...
        {
            if (_CompraID == 0 || _Modificando == true)
            {
        ...
        private void TxtDocumento_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Key == Key.Enter)
            {
                if (e.Key == Key.Enter)
                {
                    ...
                    DataTable Dt = RDocumento.BuscarDocumentoXCodigoCompras(Codigo);
                    if (Dt.Rows.Count > 0)
                    {
        ...
        private void btnBuscarProveedor_Click(object sender, RoutedEventArgs e)
        {
            ...
            if(_TerceroID!=0 && _CompraID != 0)
            {
                var RCompras = new RepositorioCompras(_ConexionSql);
                RCompras.ActualizarTerceroCompra(_CompraID, _TerceroID);
                _TerceroID=ventana.TerceroSelecionado.TerceroID;
            }
        }
        ...
        private void txtNumeroFactura_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                if(_NumeroFactura!= NumeroFactura && _CompraID!=0)
                {
                    var RCompras = new RepositorioCompras(_ConexionSql);
                    RCompras.ActualizarNumeroFacturaCompra(_CompraID, NumeroFactura);
        ...
        private void btnNuevo_Click(object sender, RoutedEventArgs e)
        {
            ...
            btnEditar.Opacity = 0.5;
            btnNuevo.Opacity = 0.5;
            btnEditar.Opacity = 0.5;
        ...
        private void dpFecha_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            {
                _FechaCompra = dpFecha.SelectedDate.Value;
                DateTime FechaCompra =dpFecha.SelectedDate.Value;
                if (_FechaCompra != FechaCompra && _CompraID!=0)
                {
                    var RCompras = new RepositorioCompras(_ConexionSql);
                    RCompras.ActualizarFechaCompra(_CompraID,FechaCompra);
        ...
        private void btnGuardar_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                btnGuardar.IsEnabled = true;
                VentanaSelecionarFormaPago Ven = new VentanaSelecionarFormaPago(1, Articulos.Sum(A=>A.TotalConIVA));
        ...
        {
            VentanaClave Ventana = new VentanaClave(1, 0, 4);
            bool? EsValido = Ventana.ShowDialog();
            if (EsValido == true)
            {
                if (_CompraID != 0 && _Modificando == true)
                {
        ...
        private void btnEliminar_Click(object sender, RoutedEventArgs e)
        {
            if (_CompraID != 0 && _Modificando == true)
            {
                VentanaClave ven = new VentanaClave(1, 0, 4);
                bool? Clave = ven.ShowDialog();
                if (Clave == true)
                {
                    var Pregunta = MessageBox.Show("¿Estas Seguro De Eliminar Comprobante?", "Pregunta", MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (Pregunta == MessageBoxResult.Yes)
                    {
                        try
                        {
                            ...
                            bool Elimnar = RCompras.ElimarCompras(_CompraID);
                            if (Elimnar == true)
                            {
                                MessageBox.Show("Compra Eliminada Correctamente", "Mensaje", MessageBoxButton.OK, MessageBoxImage.Information);
                                LimpiarCampos();
                                dgProductos.Items.Refresh();
                                ActivarBotones();
                            }
                        ...
                        {
                            MessageBox.Show("Error Al Eliminar Compra: " + ex.Message, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        }
                    }
                    else
                    {
        ...
        public ObservableCollection<ArticulosAgregados> Articulos { get; set; } = new ObservableCollection<ArticulosAgregados>();
                ...
                case Key.Enter:
                    ...
                    if (dgProductos.CurrentCell != null)
                    {
                        ...
                        if (CurrenColumnIndex < grid.Columns.Count - 1)
                        {
                            ...
                            grid.CurrentCell = new DataGridCellInfo(grid.Items[CurrentRowIndex],grid.Columns[CurrenColumnIndex + 1]);
                        }
                        else
                        {
        ...
        private void SuscribirArticulosCargados(ArticulosAgregados Items)
        {
            Items.CantidadCambiada += Items_CantidadCambiada;
            Items.ActualizarTotales += Items_ActualizarTotales;

        }

        private void Items_ActualizarTotales(object sender, EventArgs e)
        {
            CalcularTotales();
        }

        private void VentanaRegistroCompras_Loaded(object sender, RoutedEventArgs e)
        {
            if (_CompraID != 0)
                CargarCompra(_CompraID);
        }


        private void CargarCompra(int compraID)
        {
            try
            {
                var repo = new RepositorioCompras(_ConexionSql);

                Articulos = repo.ArticulosBuscados(compraID);
                dgProductos.ItemsSource = Articulos;

                DataTable dt = repo.DatosEncabezadoCompra(compraID);
                if (dt.Rows.Count > 0)
                {
                    DataRow row = dt.Rows[0];
                    TxtDocumento.Text = row["CodigoDocumento"].ToString();
                    _DocumentoID = Convert.ToInt32(row["DocumentoID"]);
                    TxtNombreDocto.Text = row["NombreDocumento"].ToString();
                    txtNumeroDocumento.Text = row["NumeroDocumento"].ToString();
                    txtNumeroFactura.Text = row["NumeroFactura"].ToString();
                    _NumeroFactura = row["NumeroFactura"].ToString();
                    _TerceroID = Convert.ToInt32(row["TercerosID"]);
                    txtProveedor.Text = row["TercerosNombreCompleto"].ToString();
                    _VendedorID = Convert.ToInt32(row["VendedorID"]);
                    cbxDigitador.SelectedValue = _VendedorID;
                    dpFecha.SelectedDate = Convert.ToDateTime(row["FechaCompra"]);
                    _FechaCompra = Convert.ToDateTime(row["FechaCompra"]);
                    txtObservaciones.Text = row["DetalleCompra"].ToString();
                    _DetalleCompra = row["DetalleCompra"].ToString();
                }
                // SUSCRIBIR EVENTOS AQUI
                foreach (var item in Articulos)
                {
                    SuscribirArticulosCargados(item);
                }
            ...
            {
                MessageBox.Show("Error al cargar la compra" + ex.ToString(), "Mensaje", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        ...
        //Resumne:
        //      el calculos de los totales como corresponde a una clase diferente a la clase del data grid ,el evento calcular totales debe ir 
        ...
        // 
        //     Importante suscribir el observable collection  al  Articulos_CollectionChanged
